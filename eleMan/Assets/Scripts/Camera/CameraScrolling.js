...
private var lockCameraTimeout = 0.2;
...
function LateUpdate () {
	...
	targetPlayerStatus = target.GetComponent (PlayerStatus);
	
	//Timeout currently only works for PlayerController
	if (!targetPlayerStatus  || (targetPlayerStatus && targetPlayerStatus.cameraTimer < lockCameraTimeout))
...
function GetGoalPosition () {
	...
	// If we find a PlayerController on the target, we can access a velocity from that!
	targetPlayerController = target.GetComponent (PlayerController);
	if (targetPlayerController)
		targetVelocity = targetPlayerController.GetVelocity ();
